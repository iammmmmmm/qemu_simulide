...
static void stm32_gpio_update_dir(Stm32Gpio *s, int cr_index) // Update the CRL or CRH Configuration Register
{
...
static void stm32_gpio_GPIOx_ODR_write( Stm32Gpio *s, uint32_t new_value )
{
    ...
    /* Update register value. Per documentation, the upper 16 bits always read as 0. */
    s->GPIOx_ODR = new_value & 0x0000ffff;

    /// We probably don't need anything from here
    /// unless there is something connected to out_irq
    /// Here we can write port Number and new state to arena

    changed = old_value ^ new_value; // Get pins that changed value
    ...
    int32_t portNumber = s->periph; // PortA = 1
    ...
    m_arena->data16 = s->GPIOx_ODR;   // 1 bit per pin: 1 = High
    m_arena->data8  = portNumber;     // We have to send Port number, PortA = 1
    m_arena->time   = qemuTime;
}
...
static uint64_t stm32_gpio_read(void *opaque, hwaddr offset, unsigned size)
{
    ...
    switch (offset) {
        case GPIOx_CRL_OFFSET:  return s->GPIOx_CRy[0];
        case GPIOx_CRH_OFFSET:  return s->GPIOx_CRy[1];
        case GPIOx_IDR_OFFSET:  qemu_set_irq( s->sync_irq[0], -1); return s->in;
